...
export const registerIpcMainListeners = (
  ...
  type SkillsDataCacheCore = Omit<NonNullable<typeof appData.skillsData>, "locs" | "icons" | "skillsDataPackPaths">;
  ...
  const getSkillAndEffectIconPaths = (
  ...
  const getLocsFromPacks = (packs: Pack[]) => {
  ...
  const loadIconsFromPacks = async (packs: Pack[], iconPaths: string[]) => {
    for (const pack of packs) {
  ...
  const loadVanillaSkillsDataCoreCachePayload = async () => {
    if (vanillaSkillsDataCoreCachePayload !== null) return vanillaSkillsDataCoreCachePayload;
    try {
      const cacheFilePath = nodePath.join(app.getPath("userData"), VANILLA_SKILLS_DATA_CORE_CACHE_FILE);
  ...
  const getVanillaSkillsDataCoreFromCache = async (dataFolder: string) => {
    ...
    const payload = await loadVanillaSkillsDataCoreCachePayload();
    if (!payload) return undefined;
    if (payload.version !== VANILLA_SKILLS_DATA_CORE_CACHE_VERSION) return undefined;
    if (payload.game !== appData.currentGame) return undefined;
  ...
  const saveVanillaSkillsDataCoreCache = async (
    dataFolder: string,
  ...
  const getDefaultSkillsSubtype = (subtypesToSet: Record<string, string[]>) => {
    if (subtypesToSet["wh_main_emp_karl_franz"]?.length) return "wh_main_emp_karl_franz";
    return Object.keys(subtypesToSet)[0];
  };

  const cloneSkillsDataCore = (core: SkillsDataCacheCore): SkillsDataCacheCore =>
    JSON.parse(JSON.stringify(core)) as SkillsDataCacheCore;

  const applyModOverlayToSkillsDataCore = (
    core: SkillsDataCacheCore,
  ...
  const getSkillsData = async (mods: Mod[]) => {
    ...
    const cachedVanillaSkillsCore = await getVanillaSkillsDataCoreFromCache(dataFolder);
    if (cachedVanillaSkillsCore) {
      ...
      const locs = getLocsFromPacks(vanillaPacks.concat(enabledModPacks));
    ...
    if (mods.length === 0) {
      void saveVanillaSkillsDataCoreCache(dataFolder, appData.skillsData);
    }
