...
export declare namespace friends {
  /**
   * Request user information for a Steam ID. This will download the user's name and avatar.
   * The information may not be available immediately. Returns true if data needs to be requested.
   *
   * @param steam_id64 - The 64-bit Steam ID of the user
   * @param require_name_only - If true, only requests the name (faster). If false, also requests avatar.
   * @returns true if data is being requested, false if data is already available
   *
   ...
   *
   * @param steam_id64 - The 64-bit Steam ID of the user
   * @returns the user's persona name, or "[unknown]" if not available yet
   *
   ...
   *
   * Note: If the information is not cached, this will return "[unknown]" on first call
   * and trigger a download. You should call this again after a short delay to get the actual name,
   * or use requestUserInformation + getFriendPersonaName separately with proper callback handling.
   *
   * @param steam_id64 - The 64-bit Steam ID of the user
...
export declare namespace workshop {
   ...
   * @returns an array of dependency item ids
   *
